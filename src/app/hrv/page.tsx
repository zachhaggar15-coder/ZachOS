...
import { ZachDualAxisChart, ZachLineChart, ZachScatterChart } from "@/components/zach-charts";
import {
  ZachButtonLink,
...
export default async function HrvPage({ searchParams }: HrvPageProps) {
  ...
  const weeklyHrvChart = weeklyFitnessTrendSeries(fitnessRows, today, 16).map(
    (row) => ({
      date: row.label,
      hrv: row.hrv,
        ...
        <>
          ...
          <section className="grid gap-4 xl:grid-cols-[1fr_1fr]">
            <ZachPanel>
              ...
              <ZachLineChart
                data={weeklyHrvChart.filter((row) => row.hrv !== null)}
                height={300}
                lines={[{ color: "#6f7d8c", key: "hrv", label: "Weekly HRV" }]}
              />
            </ZachPanel>
            <ZachPanel>
              <div className="mb-4">
              ...
              <ZachDualAxisChart
                data={weeklyHrvChart.filter(
                  (row) => row.hrv !== null || row.sleepScore !== null,
                )}
                height={300}
                lines={[
                  { color: "#6f7d8c", key: "hrv", label: "HRV", yAxisId: "left" },
                  {
                    color: "#2c2824",
                    key: "sleepScore",
                    label: "Sleep score",
