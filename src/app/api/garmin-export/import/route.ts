...
import { parseGarminExportZip } from "@/lib/garmin-export";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    parsed = parseGarminExportZip(arrayBuffer, user.id);
  } catch (error) {
    return jsonError(
      error instanceof Error
        ? error.message
        : "Could not parse that Garmin export ZIP.",
    );
  }

  if (!parsed.activities.length && !parsed.fitnessMetrics.length) {
    return jsonError(
      "No usable Garmin activity or fitness rows were found in that export.",
    );
  }
